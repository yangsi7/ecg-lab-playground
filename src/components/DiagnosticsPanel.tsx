 ...
 */
import { Activity, Database, Zap, Cpu, AlertTriangle, Wifi, X } from 'lucide-react';
...
export default function DiagnosticsPanel({ className = '' }: DiagnosticsPanelProps) {
  const { edgeFunctionStats, databaseStats, rpcMetrics, systemMetrics, isLoading, error, lastRPCCalls, connectionErrors } = useDiagnostics();
  const [connectionStatus, setConnectionStatus] = useState<ConnectionStatus>({
    isConnected: false,
    lastChecked: new Date()
  });
  const [connectionErrorsState, setConnectionErrorsState] = useState<ConnectionError[]>([]);
  const [lastRPCCallsState, setLastRPCCallsState] = useState<RPCCall[]>([]);
  ...
  return (
    <aside className={`w-full md:w-96 border-l border-white/10 p-4 bg-gray-900/50 overflow-y-auto ${className}`}>
      ...
      <div className="space-y-4">
        {/* Connection Status */}
        <div className={`rounded-xl p-4 ${connectionStatus.isConnected ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
          <div className="flex items-center gap-2">
            <Wifi className={`h-5 w-5 ${connectionStatus.isConnected ? 'text-green-400' : 'text-red-400'}`} />
        ...
        {connectionErrors && connectionErrors.length > 0 && (
              ...
              {connectionErrors.map((err: ConnectionError, index: number) => (
                <div key={index} className="text-sm text-red-300 bg-red-500/5 rounded p-2">
                  <div className="flex items-start justify-between">
                    ...
                    <span className="text-xs text-red-400">{new Date(err.timestamp).toLocaleTimeString()}</span>
                  </div>
                  {err.details && (
        ...
        {/* Last RPC Calls */}
        {lastRPCCalls && lastRPCCalls.length > 0 && (
          <div className="bg-white/5 rounded-xl p-4 space-y-2">
            ...
            <div className="space-y-2">
              {lastRPCCalls.map((call: RPCCall, index: number) => (
                <div key={index} className={`text-sm rounded p-2 ${call.error ? 'bg-red-500/10' : 'bg-white/5'}`}>
                  <div className="flex items-start justify-between">
                    <span className="text-gray-300">{call.function}</span>
                    <span className="text-xs text-gray-400">{new Date(call.timestamp).toLocaleTimeString()}</span>
                  </div>
                  {call.error ? (
                    <div className="text-xs text-red-400 mt-1">{call.error}</div>
                  ) : (
                    <div className="text-xs text-gray-400 mt-1">Duration: {call.duration}ms</div>
                  )}
                  {call.params && (
            ...
            {edgeFunctionStats.map(stat => (
              <div key={stat.function_name} className="space-y-1">
                ...
                <div className="flex justify-between text-xs text-gray-500">
                  ...
                  <span>Errors: {stat.error_count}</span>
                </div>
                {/* Success rate bar */}
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      stat.success_rate > 90 ? 'bg-green-400' :
                      stat.success_rate > 70 ? 'bg-yellow-400' :
                      'bg-red-400'
                    }`}
        ...
        <div className="bg-white/5 rounded-xl p-4 space-y-4">
          ...
          <div className="space-y-2">
            {databaseStats.map(stat => (
              <div key={stat.query_id} className="space-y-1">
                <div className="flex justify-between text-sm">
                ...
                <div className="text-xs text-gray-500">
                  Size: {(stat.size_bytes / 1024 / 1024).toFixed(2)} MB
                </div>
                {/* Cache hit ratio bar */}
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      stat.cache_hit_ratio > 0.9 ? 'bg-green-400' :
                      stat.cache_hit_ratio > 0.7 ? 'bg-yellow-400' :
                      'bg-red-400'
                    }`}
            ...
            {rpcMetrics.map(metric => (
              <div key={metric.function_name} className="space-y-1">
                ...
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      metric.error_rate < 1 ? 'bg-green-400' :
                      metric.error_rate < 5 ? 'bg-yellow-400' :
                      'bg-red-400'
                    }`}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1">
                ...
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      systemMetrics.cpu_usage < 50 ? 'bg-green-400' :
                      systemMetrics.cpu_usage < 80 ? 'bg-yellow-400' :
                      'bg-red-400'
                    }`}
              ...
              <div className="space-y-1">
                ...
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      systemMetrics.memory_usage < (256 * 1024 * 1024) ? 'bg-green-400' :
                      systemMetrics.memory_usage < (384 * 1024 * 1024) ? 'bg-yellow-400' :
                      'bg-red-400'
                    }`}
                    style={{ width: `${(systemMetrics.memory_usage / (512 * 1024 * 1024)) * 100}%` }}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-400">{systemMetrics.network_latency_ms}ms</span>
                </div>
                <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                  <div 
                    className={`h-full rounded-full transition-all ${
                      systemMetrics.network_latency_ms < 100 ? 'bg-green-400' :
                      systemMetrics.network_latency_ms < 300 ? 'bg-yellow-400' :
                      'bg-red-400'
                    }`}
                    style={{ width: `${Math.min((systemMetrics.network_latency_ms / 500) * 100, 100)}%` }}
                  />
                </div>
              </div>

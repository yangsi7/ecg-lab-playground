...
// Minimal bar chart with a simple canvas
...
export default function ClinicLab() {
    ...
    const [sortKey, setSortKey] = useState<keyof (typeof statusBreakdown)[number] | null>(null)
    ...
    const handleSort = (key: keyof (typeof statusBreakdown)[number]) => {
        if (sortKey === key) {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Quality Breakdown */}
            {!loading && !error && qualityBreakdown && (
                <div className="bg-white/10 border border-white/10 rounded-xl p-4">
                        ...
                        <table className="min-w-full text-sm text-gray-300">
                            <thead className="border-b border-white/20">
                                <tr>
                                    ...
                                    <th className="px-3 py-2 text-left">Good(&gt;70%)</th>
                                    <th className="px-3 py-2 text-left">SoSo(50-70%)</th>
                                    <th className="px-3 py-2 text-left">Bad(30-50%)</th>
                                    <th className="px-3 py-2 text-left">Critical(&lt;30%)</th>
                                </tr>
                            </thead>
                            <tbody>
                                {qualityBreakdown.map((row, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-2">{row.open_studies}</td>
                                        <td className="px-3 py-2">
                                            {(row.avg_quality_fraction * 100).toFixed(1)}%
                                        </td>
